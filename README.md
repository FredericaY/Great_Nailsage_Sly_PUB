...
一个使用 Unity 制作的 2D 动作游戏项目，整体方向偏类银河恶魔城 / 平台动作体验。当前版本已经包含角色移动与跳跃、近战连段、敌人行为树 AI、Boss 遭遇战、护符系统、商店经济、暂停菜单与输入重绑定等一整套可运行内容。

README 主要记录项目内容、运行方式、代码结构和核心机制，方便后续维护、继续扩展或直接上手阅读。
...
- 组件化玩家架构：`PlayerRoot + Input + Controller + Movement + Jump + Combat + Facing + AnimatorDriver + CharmRuntime`
- 基于动画事件的近战判定：支持连段、输入缓冲、长按重复攻击和攻击超时保护
- 行为树驱动敌人 AI：普通敌人与 Boss 共用黑板与公共底座，差异逻辑拆到自定义 Task
- 统一投射物与对象池：远程攻击围绕 `BaseProjectile2D` 和 `ProjectilePoolService` 组织
- 护符与商店系统：支持护符持有、装备、生效、购买和消耗品使用
- 暂停菜单与输入系统解耦：支持 UI / Gameplay ActionMap 切换、键位重绑定、手柄/键鼠提示
- 场景机制：包含遭遇战、Boss 生成、场景门控制、隐藏房间和基础地图切换
...
这一块整体上是一个比较完整的“定义数据 -> 保存拥有状态 -> 运行时生效 -> UI 展示 -> 商店交易”的闭环。
...
- 当前项目不只是单一战斗 Demo，而是已经包含角色、敌人、Boss、经济、UI 和场景机制的一套完整玩法样板。
- 代码结构偏模块化，公共系统和特化逻辑边界比较清晰，方便继续加角色能力、敌人类型和关卡内容。
- 数据驱动内容主要体现在护符定义、敌方攻击数据、音频 cue / bank、场景音频配置等部分。
- 一些动作游戏里常见的细节问题已经做了针对性处理，例如输入缓冲、动画事件丢失保护、玩家锁定、UI 接管输入、投射物复用等。
...
项目目前更接近“可玩的系统化原型”，核心玩法和主要支撑系统已经搭起来了，但内容量、关卡细节、数值平衡和反馈表现还有继续打磨的空间。整体上已经适合继续往关卡扩展、能力拓展和存档系统方向推进。
